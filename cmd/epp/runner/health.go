...
type healthServer struct {
	...
	// draining, when non-nil and set, forces every non-liveness check to report
	// NOT_SERVING so Kubernetes drains the pod from the Service endpoints during
	// a graceful shutdown while the ext_proc server keeps serving. Liveness stays
	// SERVING to avoid a restart mid-drain. Nil means graceful drain is disabled.
