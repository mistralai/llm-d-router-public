...
type PodCache struct {
	// cache is an LRU cache that maps pod entry value identities to their last access time.
	// thread-safe.
		...
		if pods, found := m.data.Get(requestKey); found { //nolint:nestif // TODO: can this be optimized?
			...
			if podIdentifierSet.Len() == 0 {
				...
				for _, key := range pods.cache.Keys() {
					podsPerKey[requestKey] = append(podsPerKey[requestKey], key.podEntry())
				}
			} else {
				// Filter pods based on the provided pod identifiers
				for _, key := range pods.cache.Keys() {
					if podIdentifierSet.Has(key.podIdentifier) {
						podsPerKey[requestKey] = append(podsPerKey[requestKey], key.podEntry())
					}
				}
			}
	...
	for _, requestKey := range m.data.Keys() {
		...
		for _, key := range podCache.cache.Keys() {
			if key.podIdentifier == podIdentifier && key.matchesDataParallelRank(dataParallelRank) {
				matched = append(matched, key.podEntry())
			}
		}
		podCache.mu.Unlock()
