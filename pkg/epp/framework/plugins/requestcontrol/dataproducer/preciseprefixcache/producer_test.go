...
func TestProduce_MultiPromptChoosesBestAggregateRank(t *testing.T) {
	...
	ranks, ok := scheduling.ReadRequestAttribute[map[string]int](req, routing.DataParallelWinningRanksAttribute)
	require.True(t, ok)
	assert.Equal(t, 1, ranks[addr])
	assert.NotContains(t, req.Headers, routing.DataParallelWinningRanksHeader,
		"winning-rank state must not be carried in a forwarded header")
