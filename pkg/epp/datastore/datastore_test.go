...
func TestSharedPortDataParallelRejectsMultipleTargetPorts(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			options: []Option{WithDataParallelSize(3)},
		},
		{
			name: "automatic detection",
			options: []Option{WithDataParallelSizeDetector(&fakeDataParallelSizeDetector{
				sizes:    map[string]int{},
				detected: map[string]bool{},
				errs:     map[string]error{},
			})},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ds := NewDatastore(t.Context(), &mockEndpointFactory{}, tt.options...)
...
func TestSharedPortDataParallelDetectsRanksPerPod(t *testing.T) {
	...
	assert.Nil(t, byName[pod1.Name+"-rank-0"].DataParallelRank)
	for rank := range 3 {
