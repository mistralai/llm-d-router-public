...
func TestSubscriberManager_RemoveSubscriberKeepsSharedSourceUntilLastSubscriber(t *testing.T) {
	...
	subscribers[0].enqueue(&RawMessage{Topic: "kv@", Sequence: 1, SourceEndpoint: sourceEndpoint})
	manager.RemoveSubscriber(ctx, "ns/pod-rank-0")
	subscribers[0].enqueue(&RawMessage{Topic: "kv@", Sequence: 2, SourceEndpoint: sourceEndpoint})
	subscribers[1].enqueue(&RawMessage{Topic: "kv@", Sequence: 3, SourceEndpoint: sourceEndpoint})
	require.Equal(t, 2, pool.queues[0].Len(), "removing one rank must not reset a shared source")
...
func TestZMQSubscriber_RetireDropsMessagesWithoutSourceEndpoint(t *testing.T) {
	...
	subscriber.enqueue(&RawMessage{Topic: "kv@10.0.0.1:8000@test-model", Sequence: 1})
	subscriber.retire(false)
	subscriber.enqueue(&RawMessage{Topic: "kv@10.0.0.1:8000@test-model", Sequence: 2})
